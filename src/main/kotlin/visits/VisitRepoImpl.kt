...
class VisitRepoImpl : VisitRepo {
    ...
    // Добавьте эти методы в класс VisitRepoImpl

    override suspend fun getVisitsByPatientId(patientId: Int, startDate: Instant?, endDate: Instant?): List<Visit> =
        suspendTransaction {
            val conditions = mutableListOf<Op<Boolean>>()
            conditions += Visits.patientId eq patientId
    ...
    override suspend fun getVisitsByDoctorId(doctorId: Int, startDate: Instant?, endDate: Instant?): List<Visit> =
        suspendTransaction {
            val conditions = mutableListOf<Op<Boolean>>()
            conditions += Visits.employeeId eq doctorId
    ...
    override suspend fun createVisit(visit: Visit): Visit = suspendTransaction {
        val id = Visits.insert {
            it[Visits.discountId] = visit.discount.id
            it[Visits.patientId] = visit.patient.id
            it[Visits.employeeId] = visit.employee.id
            it[Visits.visitDateAndTime] = visit.dateAndTime
